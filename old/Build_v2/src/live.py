...
def updateMagnitudeFig(figData, capture, sampleRate):
    cols = signals.accelCols(magSensor)
    timeData = signals.plotTime(capture, sampleRate, columns=cols)
    x = timeData[cols[0]]
    yAxis = timeData[cols[1]]
    z = timeData[cols[2]]
    y = np.sqrt((x * x) + (yAxis * yAxis) + (z * z))
    t = capture["t_s"].to_numpy(dtype=np.float64)
    t = t - t[0]
...
def updateFftFig(figData, capture, sampleRate):
...
def updateVisualFigs(figs, samples, sampleRate, visualRows):
    viewSamples = samples[-visualRows:]
    capture = pd.DataFrame(viewSamples, columns=data.signalCols)
    updateMagnitudeFig(figs["mag"], capture, sampleRate)
    updateFftFig(figs["fft"], capture, sampleRate)
    ...
    with serial.Serial(
        ...
        while True:
            ...
            for packetValues in packets:
                ...
                samples.append(serial_shared.packetRow(packetValues, firstTUsRaw))

            if len(samples) > keepRows:
                samples = samples[-keepRows:]
            if len(samples) < min(winRows, visualRows):
                continue
            ...
            rawProbVector = classifier.predictSamples(samples[-winRows:])
