...
def plotTime(oneSecondWindow, sampleRate, columns=None):
    if "t_s" in oneSecondWindow.columns:
        t = oneSecondWindow["t_s"].to_numpy(dtype=np.float64)
    else:
        t = np.arange(oneSecondWindow.shape[0], dtype=np.float64) / sampleRate
    out = {"t_s": t}

    if columns is None:
        columns = axisCols
    for col in columns:
        out[col] = oneSecondWindow[col].to_numpy(dtype=np.float64)
    return out


# Builds raw axis signals plus grouped magnitudes for the ML model.
def mlTimeSignals(oneSecondWindow, sampleRate):
    out = plotTime(oneSecondWindow, sampleRate)
